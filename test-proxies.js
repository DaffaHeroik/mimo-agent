...
const proxies = fs.readFileSync('/home/work/.openclaw/workspace/.openclaw/tmp/mimo-agent/proxies/all.txt','utf8').trim().split('\n').map(l => {
...
(async () => {
  for (let i = 0; i < 5; i++) {
    ...
    try {
      ...
      if (text.includes('Too many sign-ups')) console.log('  Rate limited');
      else if (text.includes("couldn't create")) console.log('  Creation error');
      else if (text.includes('already on board')) console.log('  Already registered');
      else if (page.url().includes('dashboard')) console.log('  REGISTERED!');
      else console.log('  Unknown:', text.substring(0, 150));
    } catch(e) {
