...
export async function runIbmBob(options = {}) {
  ...
  log(`\n🚀 IBM Bob Harvester — ${accounts.length} accounts\n`);
...
async function runSequential(accounts, useProxy, log) {
  const worker = new IbmBobWorker();
  const results = { success: 0, failed: 0 };
  const progress = createProgressBar('IBM Bob', accounts.length);
